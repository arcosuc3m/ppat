...
clang::SourceLocation MyASTVisitor::getEndLocation(clang::SourceLocation EndLoc,
  ...
  if (currfile == Loop.FileName) {
    ...
    auto source =
        std::string(sm.getCharacterData(EndLoc),
                    sm.getCharacterData(AuxLoc) - sm.getCharacterData(EndLoc));
  ...
  if (s != nullptr) {
    ...
    if (isa<CXXMemberCallExpr>(s)) {
      ...
      if (isa<MemberExpr>(*(s->child_begin()))) {
        ...
        // NEED TO PERFORM SOME ACTION FOR NON CONSTANT MEMBER FUNCTIONS SINCE
    ...
    if (isa<ImplicitCastExpr>(s)) {
      ...
      if (child != ice->child_end()) {
        ...
        if (isa<CallExpr>(*child)) {
          ...
          Decl *decl = call->getCalleeDecl();
          // Save argument as a function. This function call will be analyzed
          // independently.
          // if(FunctionDecl *fun = dyn_cast<clang::FunctionDecl>(decl)){
        ...
        if (isa<IntegerLiteral>(*child) || isa<StringLiteral>(*child) ||
            isa<FloatingLiteral>(*child) || isa<CXXBoolLiteralExpr>(*child)) {
          ASTVar varArg;
          varArg.Name = "Literal";
          varArg.RefLoc = 0;
          varArg.rvalue = true;
      ...
      if (isa<DeclRefExpr>(s)) {
        ...
        if (isa<IntegerLiteral>(s) || isa<StringLiteral>(s) ||
            isa<FloatingLiteral>(s) || isa<CXXBoolLiteralExpr>(s)) {
          ASTVar varArg;
          varArg.Name = "Literal";
          varArg.RefLoc = 0;
          varArg.rvalue = true;
