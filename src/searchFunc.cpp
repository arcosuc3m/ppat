...
void MyASTVisitor::searchFunc(Stmt *s,ASTLoop &Loop){
  if(s!=nullptr){
    if(isa<ForStmt>(s)){
      ...
      Loop.InternLoopsBegin.push_back(s->getSourceRange().getBegin().getRawEncoding());
      Loop.InternLoopsEnd.push_back(s->getSourceRange().getEnd().getRawEncoding());
    ...
    if(isa<CXXMemberCallExpr>(s)){
      auto call = cast<CXXMemberCallExpr>(s);
      if(isa<MemberExpr>(*(s->child_begin()))){
        ...
        Expr * obj = call->getImplicitObjectArgument ( );	
        if(Vdecl->getType().getAsString().find("const")!= std::string::npos){
          searchVarsMember(obj,Loop,false);
        }else{
          searchVarsMember(obj,Loop,false);
        }
