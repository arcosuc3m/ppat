...
void MyASTVisitor::getMapIO(std::vector<std::string> &inputs,
  ...
  for (auto refs = Loops[i].VarRef.begin(); refs != Loops[i].VarRef.end();
       refs++) {
    bool isLocal = false;
    for (auto decl = Loops[i].VarDecl.begin(); decl != Loops[i].VarDecl.end();
         decl++) {
      if ((*refs).Name == (*decl).Name) {
        isLocal = true;
      }
    }
    for (auto assign = Loops[i].Assign.begin(); assign != Loops[i].Assign.end();
         assign++) {
      if ((*assign).Name == (*refs).Name &&
          (*assign).RefLoc == (*refs).RefLoc) {
        (*refs).lvalue = true;
        (*refs).rvalue = false;
      }
    }
...
bool MyASTVisitor::checkPrivate(int i, ASTVar variable) {
  ...
  for (auto refs = Loops[i].VarRef.begin();
       refs != (Loops[i].VarRef.end() - 1) && firstOcurr; refs++) {
    if (variable.Name == (*refs).Name && variable.DeclLoc == (*refs).DeclLoc) {
      ...
      for (unsigned k = 0; k < Loops.size(); k++) {
        if (i != k &&
            Loops[i].RangeLoc.getBegin().getRawEncoding() <=
                Loops[k].RangeLoc.getBegin().getRawEncoding() &&
            Loops[i].RangeLoc.getEnd().getRawEncoding() >=
                Loops[k].RangeLoc.getEnd().getRawEncoding()) {
      ...
      if ((*refs).lvalue && !(*refs).cmpAssign) {
        ...
        while (line1 == line2) {
          if ((*refs2).Name == (*refs).Name &&
              ...
              (*refs2).RefLoc != (*refs).RefLoc && (*refs2).rvalue) {
            return false;
          }
          refs2++;
        ...
        for (unsigned codeRef = 0; codeRef < Code.VarRef.size(); codeRef++) {
          if (Code.VarRef[codeRef].Name == variable.Name &&
              variable.DeclLoc == Code.VarRef[codeRef].DeclLoc &&
              Code.VarRef[codeRef].RefLoc >
                  Loops[i].RangeLoc.getEnd().getRawEncoding()) {
            if (Code.VarRef[codeRef].lvalue)
              return true;
            else if (Code.VarRef[codeRef].rvalue)
              return false;
            else
              return true;
          }
...
bool MyASTVisitor::checkFeedback(int i, std::stringstream &SSComments,
  ...
  for (auto refs = (Loops[i].VarRef.end() - 1);
    ...
    for (auto sinkVar = Loops[i].sinkZones.begin();
         sinkVar != Loops[i].sinkZones.end(); sinkVar++) {
      if ((*refs).RefLoc >= (*sinkVar).second.first.getRawEncoding() &&
          (*refs).RefLoc <= (*sinkVar).second.second.getRawEncoding()) {
        sinkIgnore = true; /*std::cout<<"SINK IGNORE " <<std::endl;*/
      }
    }

    if (((*refs).RefLoc < Loops[i].genStart.getRawEncoding() ||
         (*refs).RefLoc > Loops[i].genEnd.getRawEncoding()) &&
        !sinkIgnore) {
      ...
      if (privated) {
        ...
        for (unsigned pv = 0; pv < Loops[i].privated.size(); pv++) {
          if ((*refs).Name == Loops[i].privated[pv])
            add = false;
        }
        if (add)
          Loops[i].privated.push_back((*refs).Name);
      }
      ...
      if ((*refs).lvalue && !privated && !local && !sinkIgnore) {
        for (auto refs2 = (Loops[i].VarRef.end() - 1);
             refs2 != (Loops[i].VarRef.begin() - 1); refs2--) {
          if ((*refs).Name == (*refs2).Name &&
              (*refs).DeclLoc == (*refs).DeclLoc &&
              ((*refs2).RefLoc < Loops[i].genStart.getRawEncoding() ||
               (*refs2).RefLoc > Loops[i].genEnd.getRawEncoding())) {
            /*bool memoryAccess = false;
            ...
            if (memoryAccess && memoryAccess2 && !sinkIgnore) {
              ...
              if((*refs2).rvalue){
                      if(Loops[i].MemAccess[mem].index.size()==Loops[i].MemAccess[mem2].index.size()){
                              feedbackVAR = true;
                              for(unsigned
              index=0;index<Loops[i].MemAccess[mem].index.size();index++){
                                      if(Loops[i].MemAccess[mem].index[index] !=
                                              Loops[i].MemAccess[mem2].index[index]){
                                              feedbackVAR = false;
                                      }
            ...
            if ((*refs2).lvalue & !memoryAccess && localVar && !sinkIgnore) {
              ...
              for (unsigned loop = 0; loop < Loops[i].InternLoops.size();
                   loop++) {
                if ((*refs).RefLoc > Loops[i]
                                         .InternLoops[loop]
                                         .getBegin()
                                         .getRawEncoding() &&
                    (*refs).RefLoc <
                        Loops[i].BodyStarts[loop].getRawEncoding()) {
                  inFor = true;
                }
              }
              SourceManager &SM = TheRewriter.getSourceMgr();
...
void MyASTVisitor::getStreamDependencies(std::vector<std::string> stream,
  ...
  for (unsigned st = 0; st < stream.size(); st++) {
    ...
    for (auto refs = (Loops[i].Assign.end() - 1);
         refs != (Loops[i].Assign.begin() - 1); refs--) {
      // If is a data stream refrence...
      if (stream[st] == (*refs).Name && !found[st]) {
        found[st] = true;
        //...Then look for the dependencies references and include them as a
        // dependencies of the stream
        for (auto refs2 = refs; refs2 != (Loops[i].Assign.begin() - 1);
             refs2--) {
          for (unsigned dep = 0; dep < (*refs).dependencies.size(); dep++) {
            ...
            if ((*refs2).Name == (*refs).dependencies[dep].Name
              ...
              for (unsigned k = 0; k < (*refs2).dependencies.size(); k++) {
                bool add = true;
                for (unsigned dep2 = 0; dep2 < (*refs).dependencies.size();
                     dep2++) {
                  if ((*refs).dependencies[dep2].Name ==
                      (*refs2).dependencies[k].Name
...
bool MyASTVisitor::checkMemAccess(std::vector<std::string> dataStreams, int i) {
  ...
  for (unsigned data = 0; data < dataStreams.size(); data++) {
    ...
    for (unsigned memAcc = 0; memAcc < Loops[i].MemAccess.size() && notFound;
         memAcc++) {
      if (dataStreams[data] == Loops[i].MemAccess[memAcc].Name) {
        // Check if every access is on the same position. If not return false.
        for (unsigned memAcc2 = 0;
             memAcc2 < Loops[i].MemAccess.size() && notFound; memAcc2++) {

          if (Loops[i].MemAccess[memAcc].index.size() !=
              Loops[i].MemAccess[memAcc2].index.size()) {
            return false;
          } else {
            for (unsigned in = 0; in < Loops[i].MemAccess[memAcc].index.size();
                 in++) {
              if (Loops[i].MemAccess[memAcc].index[in] !=
                  Loops[i].MemAccess[memAcc2].index[in])
                return false;
            }
          }
        }
...
bool MyASTVisitor::globalLValue(int i) {
  ...
  for (auto refs = Loops[i].VarRef.begin(); refs != Loops[i].VarRef.end();
    ...
    if ((Loops[i].genEnd.getRawEncoding() <=
             (*refs).RefSourceLoc.getRawEncoding() ||
         (*refs).RefSourceLoc.getRawEncoding() <
  ...
  for (auto calls = Loops[i].fCalls.begin(); calls != Loops[i].fCalls.end();
    ...
    for (auto sinkVar = Loops[i].sinkZones.begin();
         sinkVar != Loops[i].sinkZones.end(); sinkVar++) {
      if ((*calls).SLoc.getRawEncoding() >=
              (*sinkVar).second.first.getRawEncoding() &&
          (*calls).EndLoc.getRawEncoding() <=
              (*sinkVar).second.second.getRawEncoding())
        sinkIgnore = true;
    }

    if ((Loops[i].genEnd.getRawEncoding() <= (*calls).SLoc.getRawEncoding() ||
         (*calls).SLoc.getRawEncoding() < Loops[i].genStart.getRawEncoding()) &&
        !sinkIgnore) {
...
bool MyASTVisitor::check_break(int i) {
  ...
  for (unsigned brk = 0; brk < Loops[i].Breaks.size(); brk++) {
    ...
    for (unsigned internal = 0; internal < Loops[i].InternLoops.size();
         internal++) {
      if (Loops[i].Breaks[brk].getRawEncoding() >
              Loops[i].InternLoops[internal].getBegin().getRawEncoding() &&
          Loops[i].Breaks[brk].getRawEncoding() >
              Loops[i].InternLoops[internal].getEnd().getRawEncoding()) {
        isInternal = true;
      }
    }
    ...
    */
    ...
    if (fileName == currfile) {
      ...
      if (!invalid && !feedback && !globallvalue && !hasreturn && !empty &&
        ...
        //       auto fileName =
        //       SM.getFileEntryForID(SM.getDecomposedLoc(Loops[i].RangeLoc.getBegin()).first)->getName();
        SSBefore << "== Loop in " << fileName << ":" << line
                 << " does not match a map pattern!\n";

        if (feedback)
          SSBefore << "\tFeedback detected.\n ";
        if (invalid)
          SSBefore << "\tOutput does not depend on any Input\n";
        if (globallvalue)
