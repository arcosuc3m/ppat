...
static llvm::cl::opt<bool> PoolOption("pool",llvm::cl::desc("Detect and analyze pipelines"),llvm::cl::cat(ToolingSampleCategory));
static llvm::cl::opt<bool> WFarmOption("wfarm",llvm::cl::desc("Detect and analyze pipelines"),llvm::cl::cat(ToolingSampleCategory));
static llvm::cl::opt<bool> MapOption("map",llvm::cl::desc("Detect and analyze map patterns"), llvm::cl::cat(ToolingSampleCategory));
static llvm::cl::opt<bool> StencilOption("stencil",llvm::cl::desc("Detect and analyze stencil patterns"), llvm::cl::cat(ToolingSampleCategory));
static llvm::cl::opt<bool> ConvolutionOption("convolution",llvm::cl::desc("Detect and analyze stencil patterns"), llvm::cl::cat(ToolingSampleCategory));
...
class MyASTVisitor : public RecursiveASTVisitor<MyASTVisitor> {
  ...
  public:
    ...
    //POOL DETECTOR
    bool checkPool(int i,int stages,std::vector<unsigned>  stagesLocEnds);
    bool PoolDetect();
    ...
    //STENCIL DETECTOR
    std::vector<std::string> getOtherIterators ( int i);
    bool checkNeightbor(std::vector<std::basic_string<char> >&,std::vector<std::string> iterators, int);
    std::vector<std::string> checkKernel(std::vector<std::string>& inputs,std::vector<std::string> iterators, int i);
    void stencilDetect();
    //WINDOW FARM
    bool windowDetector(std::vector<std::string> feedvars, int i);
    bool checkFeedbackLoop(int i,int stages,std::vector<unsigned> stagesLocEnds, std::vector<std::string>& feedvars, int &win, int &update);
    bool checkWindow(int i, std::vector<unsigned> stagesLocEnds,int win, std::vector<std::string> feedvars);
    bool WFarmDetect();  
          ...
          for(auto decIt = dec->decl_begin();decIt!=dec->decl_end();decIt++){
            if((*decIt)!=NULL){
              if(VarDecl *var = cast<VarDecl>((*decIt))){
                ...
                Kernel.Iterator.type = clang::QualType::getAsString(var->getType().split());                                                                            
    ...
    ~MyASTVisitor(){
      if(!firstIteration){
        ...
        std::cout<<"\n\nStarting stencil detection phase... \n";
        if(StencilOption) stencilDetect();
        std::cout<<"\n\nStarting farm detection phase... \n";
        if(FarmOption) farmDetect();
        std::cout<<"\n\nStarting reduce detection phase...\n";
        if(ReduceOption) reduceDetect();
        std::cout<<"\n\nStarting window farm detection phase...\n";
        if(WFarmOption) WFarmDetect();
        std::cout<<"\n\nStarting pool detection phase...\n";
        if(PoolOption) PoolDetect();
