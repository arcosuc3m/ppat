...
bool MyASTVisitor::varsUnaryOperator(Stmt *s, Struct &Loop){
				...
				*/	if(isa<DeclRefExpr>(*children)){
						...
						if(isa<CXXThisExpr>(*children)) return false;
...
template <typename Struct> 
bool MyASTVisitor::RecursiveCXXOperatorCallExpr(Stmt *s, ImplicitCastExpr * Expr, Struct &Loop){
	int childs = 0;
	if(s!=nullptr){
...
bool MyASTVisitor::varsArraySubscript(Stmt *s, Struct &Loop){
				...
				if(isa<MemberExpr>((*UoExpr->child_begin()))){
					...
					if(isa<ImplicitCastExpr>((*member->child_begin()))){
						ImplicitCastExpr * Imp = cast<ImplicitCastExpr>((*member->child_begin()));
						//Get attribute array
						if(isa<DeclRefExpr>((*Imp->child_begin()))){
							...
							if(isa<VarDecl>(ref->getDecl())){
								...
								//var->dump();
//								std::stringstream stream;
//								stream << var->getNameAsString() << MemberExpr;
...
template <typename Struct>
bool MyASTVisitor::recursiveArraySubscript(Stmt *s,ArraySubscriptExpr * uo,Struct &Loop){
...
void MyASTVisitor::BinaryOperators(Stmt *s, Struct& Loop){
	if(s!=nullptr){
		if(s->children().begin() != s->children().end()){
		if(isa<CompoundAssignOperator>(s)||isa<UnresolvedMemberExpr>((*s->child_begin()))||isa<CXXDependentScopeMemberExpr>((*s->child_begin()))){
        ...
        //                        llvm::errs()<<mem->getMemberDecl()->getNameAsString()<<"\n";            
			...
			if(isa<DeclStmt>(s)||isa<DeclRefExpr>(s)){
				...
				if(valid) Loop.Assign.push_back(newVarRef);
			}else{
...
*/
template <typename Struct>
bool MyASTVisitor::varsCompoundAssignment(CompoundAssignOperator * cmp, Stmt *s,Struct &Loop){
	if(s!=nullptr){
		bool firstOp = true;
		bool found = false;
		for(Stmt::child_iterator child = s->child_begin();child!=s->child_end();child++){
			...
			if(firstOp){
				if(isa<DeclRefExpr>(*child)){
				 DeclRefExpr *ref = cast<DeclRefExpr>(*child);
						  ...
						  //if(clang::BinaryOperator::isMultiplicativeOp(cmp->getOpcode()) || clang::BinaryOperator::isAdditiveOp(cmp->getOpcode())){

						  if(!cmp->getOpcodeStr(cmp->getOpcode()).empty()){
						     if(cmp->getOpcodeStr(cmp->getOpcode()).str().find("+=") != std::string::npos ) newVarRef.cmpOper = "+";
						     else if(cmp->getOpcodeStr(cmp->getOpcode()).str().find("-=")!= std::string::npos) newVarRef.cmpOper = "-";
						     else if(cmp->getOpcodeStr(cmp->getOpcode()).str().find("*=")!= std::string::npos) newVarRef.cmpOper = "*";
						     else if(cmp->getOpcodeStr(cmp->getOpcode()).str().find("/=")!= std::string::npos) newVarRef.cmpOper = "/";
			                                 ...
			                                 DeclRefExpr *ref = cast<DeclRefExpr>(*child);
                        		                if(ref->getDecl()!=nullptr){
                                        		        ...
                                        		        if(VarDecl *var = dyn_cast<clang::VarDecl>(ref->getDecl())){
			                                                  ...
			                                                  newVarRef.cmpAssign = true;
						  			  //newVarRef.cmpOper = cmp->getOpcodeStr(cmp->getOpcode());	
						  			  /*if(cmp->isMultiplicativeOp(cmp->getOpcode()) || cmp->isAdditiveOp(cmp->getOpcode())){
						  			    if(cmp->getOpcodeStr(cmp->getOpcode()).find("+=")) newVarRef.cmpOper = "+";
									    if(cmp->getOpcodeStr(cmp->getOpcode()).find("-=")) newVarRef.cmpOper = "-";
									    if(cmp->getOpcodeStr(cmp->getOpcode()).find("*=")) newVarRef.cmpOper = "*";
									    if(cmp->getOpcodeStr(cmp->getOpcode()).find("/=")) newVarRef.cmpOper = "/";
									  }else newVarRef.cmpOper = "?";*/
						...
						if(isa<MemberExpr>(*child)){
						   ...
						   MemberExpr * mem = cast<MemberExpr>(*child);
								...
								Var.cmpAssign = true;
						  		...
						  		if(!cmp->getOpcodeStr(cmp->getOpcode()).empty()){
								     if(cmp->getOpcodeStr(cmp->getOpcode()).str().find("+=") != std::string::npos ) Var.cmpOper = "+";
								     else if(cmp->getOpcodeStr(cmp->getOpcode()).str().find("-=")!= std::string::npos) Var.cmpOper = "-";
								     else if(cmp->getOpcodeStr(cmp->getOpcode()).str().find("*=")!= std::string::npos) Var.cmpOper = "*";
								     else if(cmp->getOpcodeStr(cmp->getOpcode()).str().find("/=")!= std::string::npos) Var.cmpOper = "/";
								     else Var.cmpOper = "?";
						  		}
...
/**
	...
	@param[in]	s, pointer to the analyzed statement.
	@param[in,out]	Loop, ASTLoop struct to store every variable information.
...
/**
        ...
        variable declaration or reference and it will store 
        name, location, type and kind (lvalue,rvalue,xvalue and glvalue).
	Used to collect variable informations for user-defined and accesible functions.

        @param[in]      s, pointer to the analyzed statement.
        @param[in,out]  Loop, ASTFunctionDecl struct to store every variable information.
