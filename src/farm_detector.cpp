...
bool MyASTVisitor::checkEmptyTask(int i) {
  ...
  for (auto stmt = Loops[i].stmtLoc.begin(); stmt != Loops[i].stmtLoc.end();
       stmt++) {
    bool isTask = true;
    if ((*stmt).getBegin().getRawEncoding() >=
            Loops[i].genStart.getRawEncoding() &&
        Loops[i].genEnd.getRawEncoding() > (*stmt).getBegin().getRawEncoding())
      isTask = false;

    for (auto sink = Loops[i].sinkZones.begin();
         sink != Loops[i].sinkZones.end(); sink++) {
      if ((*stmt).getBegin().getRawEncoding() >=
              (*sink).second.first.getRawEncoding() &&
          (*sink).second.second.getRawEncoding() >
              (*stmt).getBegin().getRawEncoding())
        isTask = false;
    }
    if (isTask)
      empty = false;
...
void MyASTVisitor::getPrivated(int i) {
  for (auto refs = (Loops[i].VarRef.end() - 1);
    ...
    if (privated) {
      Loops[i].privatedVars.insert(
          std::pair<std::string, ASTVar>((*refs).Name, (*refs)));
    }
  }
}
...
/**
   ...
   This function computes the code regions potentially related to the sink
   lambda function.
   It takes as sink variables, those variables that are shared among different
   computing
   entities and written, not related to the generation part. Then it checks if
   the variable
   is not used before the first write nor after the las write. If both
   requirements are met
   the code region can be used as a sink code region.

   Then this function computes which variables used in the sink zones are
   modified or comes from
   previous phases. Note that those variables cannot be the same as the "sink
   variables".
...
bool MyASTVisitor::getSinkFunction(int i) {
    ...
    if (!local && !privated && (*refs).lvalue) {
      ...
      for (auto sink = sinkVars.begin(); sink != sinkVars.end(); sink++) {
        if ((*sink).first == (*refs).Name) {
          if ((*sink).second.first.getRawEncoding() > (*refs).RefLoc)
            (*sink).second.first = (*refs).RefSourceLoc;
          if ((*sink).second.second.getRawEncoding() < (*refs).RefLoc)
            (*sink).second.second = (*refs).RefSourceLoc;
          inserted = true;
        }
      }
      if (!inserted) {
        sinkVars.insert(
            std::pair<std::string,
                ...
                (*refs).Name,
                std::pair<clang::SourceLocation, clang::SourceLocation>(
                    (*refs).RefSourceLoc, (*refs).RefSourceLoc)));
      }
    }
  }
  ...
  for (auto sink = sinkVars.begin(); sink != sinkVars.end(); sink++) {
    ...
    for (auto stmt = Loops[i].stmtLoc.begin(); stmt != Loops[i].stmtLoc.end();
         stmt++) {
      if ((*sink).second.second.getRawEncoding() <
          (*stmt).getBegin().getRawEncoding()) {
        (*sink).second.second = (*stmt).getBegin();
        endStmt = false;
        break;
      }
    }
    for (auto stmt = Loops[i].stmtLoc.end() - 1;
         stmt != Loops[i].stmtLoc.begin() - 1; stmt--) {
      if ((*stmt).getEnd().getRawEncoding() <
          (*sink).second.first.getRawEncoding()) {
        (*sink).second.first = (*(stmt + 1)).getBegin();
        break;
      }
    }
    if (endStmt)
      (*sink).second.second = Loops[i].RangeLoc.getEnd();
  }
  // If the modified variable is never used after the last write or before the
  // last write, its a sink variable
  bool sinkZone = false;
  for (auto sink = sinkVars.begin(); sink != sinkVars.end(); sink++) {
    bool validZone = true;
    for (auto refs = Loops[i].VarRef.begin(); refs != Loops[i].VarRef.end();
         refs++) {
      if ((*sink).first == (*refs).Name &&
          (*refs).RefLoc < (*sink).second.first.getRawEncoding())
        validZone = false;
      if ((*sink).first == (*refs).Name &&
          (*refs).RefLoc >= (*sink).second.second.getRawEncoding())
        validZone = false;
    }
    if (validZone) {
  ...
  if (sinkZone) {
    ...
    for (auto refs = Loops[i].VarRef.begin(); refs != Loops[i].VarRef.end();
      ...
      for (auto sink = Loops[i].sinkZones.begin();
           sink != Loops[i].sinkZones.end(); sink++) {
        if ((*refs).RefLoc >= (*sink).second.first.getRawEncoding() &&
            (*refs).RefLoc < (*sink).second.second.getRawEncoding()) {
          used = true;
        }
      }
      if (used)
        sinkUsedVars.insert(
            std::pair<std::string, std::string>((*refs).Name, (*refs).type));
    }
    ...
    for (auto refs = Loops[i].VarRef.begin(); refs != Loops[i].VarRef.end();
      ...
      for (auto sink = Loops[i].sinkZones.begin();
        ...
        if ((*refs).RefLoc < (*sink).second.first.getRawEncoding() ||
            (*refs).RefLoc >= (*sink).second.second.getRawEncoding()) {
          for (auto used = sinkUsedVars.begin(); used != sinkUsedVars.end();
               used++) {
...
void MyASTVisitor::getStreamGeneratorOut(int i) {
  ...
  for (auto var = Loops[i].VarRef.begin(); var != Loops[i].VarRef.end();
       var++) {
    if ((*var).lvalue && (*var).RefLoc >= Loops[i].genStart.getRawEncoding() &&
        (*var).RefLoc < Loops[i].genEnd.getRawEncoding())
      modifiedVars.push_back((*var));
  }
  for (auto condVar = Loops[i].cond.VarRef.begin();
       condVar != Loops[i].cond.VarRef.end(); condVar++) {
    if ((*condVar).lvalue)
  ...
  for (auto cond = modifiedVars.begin(); cond != modifiedVars.end(); cond++) {
    for (auto var = Loops[i].VarRef.begin(); var != Loops[i].VarRef.end();
      ...
      if ((*var).Name == (*cond).Name) {
        if ((*var).RefLoc < Loops[i].genStart.getRawEncoding() ||
            (*var).RefLoc >= Loops[i].genEnd.getRawEncoding())
          generatorVars.insert(
              std::pair<std::string, std::string>((*cond).Name, (*cond).type));
      }
    }
  }
...
/**
  ...
  Returns all the modified variables inside the code between fWrite and lWrite.
  This function is used to know
  if there are more generator variables.
*/
std::vector<ASTVar> MyASTVisitor::getOtherGenerations(int i,
                                                      SourceLocation &fWrite,
                                                      SourceLocation &lWrite) {
  SourceManager &sm = TheRewriter.getSourceMgr();
  clang::LangOptions lopt;
  clang::BeforeThanCompare<clang::SourceLocation> isBefore(sm);
  ...
  for (auto var = Loops[i].VarRef.begin(); var != Loops[i].VarRef.end();
    ...
    /*       for(auto priv = Loops[i].privatedVars.begin(); priv !=
       Loops[i].privatedVars.end(); priv++){
  ...
  for (auto var = Loops[i].VarRef.begin(); var != Loops[i].VarRef.end();
       var++) {
    for (auto mod = modified_variables.begin(); mod != modified_variables.end();
         mod++) {
  ...
  for (auto cond = streamGenerationRefs.begin();
       cond != streamGenerationRefs.end(); cond++) {
    // std::cout<<"GENERATION VARIABLE "<< (*cond).Name<<"\n";
    if ((*cond).lvalue) {
      if (isBefore((*cond).RefSourceLoc, fWrite)) {
        fWrite = (*cond).RefSourceLoc;
      }
      if (isBefore(lWrite, (*cond).RefSourceLoc))
        lWrite = (*cond).RefSourceLoc;
    }
  }
...
bool MyASTVisitor::getStreamGeneration(int i) {
  ...
  // TODO: Its necessary to add in this part the variables modified by an
  // unknown function. If the variable modified by a unknown fucntion its never
  // used or does not depends on the actual generation variables should be
  // discarded and probably added as a sink variable. Also, its necesary to add
  // as stream generation variables, those variables used in a if condition that
  // perfroms a brek inside its body. For including multiple zones is required
  // to detect where the different variables are used in the bode (before or
  // after). //TODO: GOOD LUCK FUTURE ME!
  std::vector<ASTVar> condition_variables;
  // Get first and last writes of the modified condition variables
  // Reads are only allowed between both first and last writes and only after or
  // before the write section. Otherwise is not a farm, since the stream
  // generation is iteration dependent.
  for (auto var = Loops[i].VarRef.begin(); var != Loops[i].VarRef.end();
       var++) {
    for (auto condVar = Loops[i].cond.VarRef.begin();
         condVar != Loops[i].cond.VarRef.end(); condVar++) {
  ...
  clang::LangOptions lopt;
  clang::BeforeThanCompare<clang::SourceLocation> isBefore(sm);
  for (auto cond = condition_variables.begin();
       cond != condition_variables.end(); cond++) {
    if ((*cond).lvalue) {
      // std::cout<< "CONDITION : " << (*cond).Name << std::endl;
      if (isBefore(firstWrite, (*cond).RefSourceLoc)) {
        firstWrite = (*cond).RefSourceLoc;
      }
      if (isBefore((*cond).RefSourceLoc, lastWrite))
        lastWrite = (*cond).RefSourceLoc;
    }
  }
  // Recompute the code region related to the stream generation formula until
  // the zone doesn't change.
  ...
  do {
    ...
    for (auto stmt = Loops[i].stmtLoc.begin(); stmt != Loops[i].stmtLoc.end();
         stmt++) {
      if (lWrite.getRawEncoding() < (*stmt).getBegin().getRawEncoding()) {
        lWrite = (*stmt).getBegin();
        endStmt = false;
        break;
      }
    }
    for (auto stmt = Loops[i].stmtLoc.end() - 1;
         stmt != Loops[i].stmtLoc.begin() - 1; stmt--) {
      if ((*stmt).getEnd().getRawEncoding() <= fWrite.getRawEncoding()) {
        fWrite = (*(stmt + 1)).getBegin();
        break;
      }
    }
    if (endStmt)
      lWrite = Loops[i].RangeLoc.getEnd();
    // std::cout<<"LOCATIONS : " << fWrite.getRawEncoding() << "
    // "<<lWrite.getRawEncoding() << std::endl;
    // for(auto cond = condition_variables.begin(); cond !=
    // condition_variables.end();cond++){
    // std::cout<<"CONDITION VARIABLES : " << (*cond).Name<<"
    // "<<(*cond).lvalue<< " "<<(*cond).RefSourceLoc.getRawEncoding()
    // <<std::endl;
    //}
    ...
    // condition_variables.end();cond++){
    //     std::cout<<"CONDITION VARIABLES : " << (*cond).Name<<"
    //     "<<(*cond).lvalue<< std::endl;
    //}
  } while (fWrite.getRawEncoding() != firstWrite.getRawEncoding() &&
           lWrite.getRawEncoding() != lastWrite.getRawEncoding());
  ...
  bool before = false;
  bool after = false;
  if (firstWrite == Loops[i].bodyRange.getBegin() &&
      lastWrite == Loops[i].RangeLoc.getEnd())
    return true;
  for (auto cond = condition_variables.begin();
       cond != condition_variables.end(); cond++) {
    if (isBefore((*cond).RefSourceLoc, firstWrite))
      before = true;
    if (isBefore(lastWrite, (*cond).RefSourceLoc))
      after = true;
  }
  ...
  // If the generation variables are not used in the compute or used after and
  // before their modification, the loop cannot be transformed into a farm
  // pattern.
  // std::cout<<"AFTER : " << after << " BEFORE: "<<before << std::endl;

  if (after && before) {
    Loops[i].genStart = Loops[i].bodyRange.getBegin();
    Loops[i].genEnd = Loops[i].bodyRange.getBegin();
  }
  // if(!after&&!before){Loops[i].genStart = Loops[i].bodyRange.getBegin();
  // Loops[i].genEnd = Loops[i].bodyRange.getBegin();}
...
void MyASTVisitor::farmDetect() {
  ...
  for (int i = 0; i < numLoops; i++) {
    ...
    if (fileName == currfile) {
      ...
      // Check if output streams are only used as write or read in the same
      // position.
      ...
      if (Loops[i].numOps == 0)
        empty = true;
      //              if(Loops[i].genStart == Loops[i].RangeLoc.getBegin() &&
      //              Loops[i].genEnd == Loops[i].RangeLoc.getEnd() && !valid)
      //              empty = true;
      ...
      for (auto funs = Loops[i].fCalls.begin(); funs != Loops[i].fCalls.end();
        ...
        if ((*funs).Name.find("fscanf") != std::string::npos)
          valid = false;
        if ((*funs).Name.find("strtok") != std::string::npos)
          valid = false;
        if ((*funs).Name.find("fgets") != std::string::npos)
          valid = false;
      }
      ...
      //----------------------------
      if (!feedback && !globallvalue && !hasreturn && !empty && !hasbreak &&
          !hasgoto && valid) {
        numFarm++;
        Loops[i].farm = true;
        // ADD I/O IN LOOPS
        ...
        //                        auto line =
        //                        SM.getLineNumber(SM.getDecomposedSpellingLoc(Loops[i].RangeLoc.getBegin()).first,Loops[i].RangeLoc.getBegin().getRawEncoding());

        auto line = SM.getPresumedLineNumber(Loops[i].RangeLoc.getBegin());
        // SSBefore<< "== INVALID PATTERN == Invalid farm on
        // "<<fileName<<":"<<line<<"\n";
        SSBefore << "== Loop in " << fileName << ":" << line
                 << " does not match a farm pattern!\n";
        if (feedback)
          SSBefore << "\tFeedback detected.\n ";
        if (globallvalue)
          SSBefore << "\tFound a write on a global variable.\n";
        if (hasreturn)
          SSBefore << "\tFound a return statement.\n";
        if (empty)
          SSBefore << "\tFound no parallelizable statements.\n";
        if (hasbreak)
