                ...
                //Check if is an assignement
         		for(unsigned assign = 0; assign < Loops[i].Assign.size(); assign++){
		            	...
		            	Loops[i].Assign[assign].RefLoc == Loops[i].VarDecl[dec].DeclLoc ){
                                //llvm::errs()<<"WE HAVE TO REMOVE TYPE BEFORE VARIABLE DECLARATION TO MOVE IT\n";
                                assigned = true;
                        }
                }
                                        ...
                                        for(auto refs = Code.VarRef.begin();refs!= Code.VarRef.end();refs++){
                                                if((*arg).Name == (*refs).Name && (*arg).RefLoc == (*refs).RefLoc){
                                                       ...
                                                       (*refs).rvalue = true;
                                                        ...
                                                        if((*funs).paramTypes[paramIt].find("Output")!=std::string::npos){                                                                                              //Set as Lvalue
                                                                      (*refs).lvalue = true;
                                                                      (*refs).rvalue = false;
  					        ...
  					        bool declaration = false;
                                                ...
                                                for(unsigned dec= 0; dec<Loops[i].VarDecl.size();dec++){
                                                        if(Loops[i].VarDecl[dec].Name == (*refs).Name &&
                                                                ...
                                                                Loops[i].VarDecl[dec].RefLoc == (*refs).RefLoc){
                                                                declaration  = true;
                                                        }
                                                }
                                                if(!declaration){
...
bool MyASTVisitor::checkFeedback(int i, std::stringstream &SSComments, bool &comments, int stages , std::vector<unsigned> stagesLocEnds){
                ...
                for(auto refs = (Loops[i].VarRef.end()-1);refs != (Loops[i].VarRef.begin()-1);refs--){
						...
						//Check if is a memory access. 
                                                //As we can't detect memory collisions we exclude them from this checking
...
/*		auto lineStart = SM.getLineNumber(SM.getDecomposedLoc(Loops[i].RangeLoc.getBegin()).first,Loops[i].RangeLoc.getBegin().getRawEncoding());
	...
	//	SourceManager &SM = TheRewriter.getSourceMgr();*/
		...
		//TODO: TO BE IMPLEMENTED (MULTIPLEITERATIONS) On multiple iterations, this is the only one function to be executed on the first iteration.  
		//llvm::errs()<<"ANALYZE CALLS\n";
		analyzeCall(i);

		//---------------
/*		for(auto funs = Functions.begin(); funs!= Functions.end();funs++){
			for(auto arguments = (*funs).Arguments.begin();arguments != (*funs).Arguments.end(); arguments++){
			}		
		}
*/
...
//		bool globallvalue = false;
		...
		bool feedback = false;
		//bool feedback = checkFeedback(i,SSComments,comments,stages,stagesLocEnds);
