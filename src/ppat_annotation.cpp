    ...
    if (Loops[i].map || Loops[i].farm || Loops[i].reduce) {
      ...
      if (!omp) {
        ...
        thread launch tasks
                ...
                SSEnd<<"}\n";//End single
                SSEnd<<"#pragma omp taskwait\n}\n";//Wait and end parallel
        section
                TheRewriter.InsertText(Loops[i].InitLoc, SSTask.str(),
        true,true);
                TheRewriter.InsertText(Loops[i].RangeLoc.getEnd(), SSEnd.str(),
        true, true);
        ...
        if (Loops[i].reduce && outer) {
          ...
          for (unsigned red = 0; red < Loops[i].reduceVar.size(); red++) {
            SSBefore << " reduction(" << Loops[i].operators[red] << ":"
                     << Loops[i].reduceVar[red] << ")";
          }
          SSBefore << "\n";
          TheRewriter.InsertText(Loops[i].RangeLoc.getBegin(), SSBefore.str(),
                                 true, true);
