template <typename T> void MyASTVisitor::searchMemAcc(Stmt *s, T &Loop) {
  if (s != nullptr) {
    bool found = false;
    //		s->dump(i);
    // WORK IN PROGRESS
...
var->getNameAsString();
                                                        newMemAcc.RefLoc =
ref->getLocStart().getRawEncoding();
                                                }
                                        }
                                }
                                ...
                                //Multidimensional array
                                        valid &=
searchArrayAcc(*(imCast->child_begin()),Loop,newMemAcc);
                                }else{
                                        //Not valid for now
                                        valid = false;
                                }
                ...
                if(isa<ImplicitCastExpr>(*child2)){
                        ImplicitCastExpr * imCast =
cast<ImplicitCastExpr>((*child));
                        for(Stmt::child_iterator imchild =
imCast->child_begin();imchild!=imCast->child_end();imchild++){
...
*/
template <typename T>
bool MyASTVisitor::searchArrayAcc(Stmt *s, T &Loop,
                                  ASTMemoryAccess &newMemAcc) {
  // child = operator
  // child 2 = iterator
  Stmt::child_iterator child2 = s->child_begin();
  ...
  if (*child != nullptr && *child2 != nullptr) {
    ...
    //	if(isa<ImplicitCastExpr>(*child2)){
    //		ImplicitCastExpr * imCast = cast<ImplicitCastExpr>((*child));
    //	        for(Stmt::child_iterator child =
    //->child_begin();imchild!=imCast->child_end();imchild++){
...
*/
template <typename T> bool MyASTVisitor::searchMemOperator(Stmt *s, T &Loop) {
  if (isa<CXXOperatorCallExpr>(s)) {
      ...
      if (isa<CXXMethodDecl>(decl)) {
        ...
        if (fun->getNameAsString().find("operator[]") != std::string::npos) {
          ...
          for (int i = 0; i < numArgs; i++) {
            ...
            // nt dimension = 0;
            if (i == 1)
              getIterator(arg, Loop, newMemAcc);
            if (i == 0)
              getOperator(arg, Loop, newMemAcc, dimension);
          }
...
template <typename T>
bool MyASTVisitor::getOperator(Stmt *s, T &Loop, ASTMemoryAccess &newMemAcc,
                               int &dimension) {
  if (s != nullptr) {
    if (isa<CXXOperatorCallExpr>(s)) {
      dimension++;
