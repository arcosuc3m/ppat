...
bool MyASTVisitor::checkFeedback(int i, std::vector<std::string> &feedbackVars, std::vector<std::string> &operators){
                bool feedback = false;
			...
			/*		bool add = false;
				...
				if((*refs).cmpAssign&&!memoryAccess&&!inFor&&!privated){
					  ...
					  operators.push_back((*refs).cmpOper);
                                ...
                                for(auto refs2 = refs-1; refs2 != (Loops[i].VarRef.begin()-1) && !privated;refs2--){
                                        if((*refs).Name == (*refs2).Name && (*refs).DeclLoc == (*refs).DeclLoc){
...
bool MyASTVisitor::reduceOperation(std::vector<std::string> feedbackVars, std::vector<std::string> outputs, std::vector<std::string> &reduceVars){
	bool reduce = false;
			...
			numreduce++;
				...
				if ( outputs.size() > 0 ){
	                                ...
	                                for(int out = 0; out < outputs.size(); out++){ 
                        	                 ...
                        	                 if (out != outputs.size()-1 ) SSBefore<< ", ";
                	                }
					SSBefore <<")";
	                        }
	                        //ADD REDUCE OUTPUTS
				if ( reduceVars.size() > 0 ){
					...
					for ( int red = 0; red < reduceVars.size(); red++ ){
						...
						if( red != reduceVars.size() - 1 ) SSBefore << ", ";
					}
					SSBefore << ") ";
				}
